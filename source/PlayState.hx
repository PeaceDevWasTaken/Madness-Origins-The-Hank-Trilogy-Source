...
{
	var debugObjects:Array<FlxSprite> = [];
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			case 'stage': // Week 1
				...
				camPosThingX = stageFront.x + stageFront.width / 2;
				camPosThingY = stageFront.y - 100;
			...
			case 'spooky': // Week 2
				...
				FlxG.camera.setScrollBoundsRect(halloweenBG.x, halloweenBG.y, halloweenBG.width, halloweenBG.height);
				defaultCamZoom = .7;
				camPosThingX = halloweenBG.x + halloweenBG.width / 2;
				camPosThingY = halloweenBG.y + halloweenBG.height / 2;
			...
			case 'philly': // Week 3
				...
				FlxG.camera.setScrollBoundsRect(city.x, city.y, city.width, city.height);
				...
				add(phillyStreet);
				camPosThingX = phillyStreet.x + phillyStreet.width / 2;
				camPosThingY = phillyStreet.y + phillyStreet.width / 2;
			...
			case 'limo': // Week 4
				...
				//	FlxG.camera.setScrollBoundsRect(skyBG.x, skyBG.y, skyBG.width, skyBG.height);

				camPosThingX = skyBG.x + skyBG.width / 2;
				camPosThingY = skyBG.y + skyBG.height / 2 - 100;
			...
			case 'mall': // Week 5 - Cocoa, Eggnog
				...
				camPosThingX = bg.x + bg.width / 2;
				camPosThingY = bg.y + bg.height / 2 + 200;
			...
			case 'mallEvil': // Week 5 - Winter Horrorland
				...
				bg.updateHitbox();
				add(bg);
				FlxG.camera.setScrollBoundsRect(bg.x, bg.y, bg.width, bg.height + 300);
				defaultCamZoom = .65;
				...
				var evilSnow:BGSprite = new BGSprite('christmas/evilSnow', -500, 650);
				add(evilSnow);
			...
			case 'school': // Week 6 - Senpai, Roses
				...
				// FlxG.camera.setScrollBoundsRect(bgStreet.x, bgStreet.y, bgStreet.width, bgStreet.height);

				camPosThingX = bgStreet.x + bgStreet.width / 2;
				camPosThingY = bgStreet.y - 50;
				...
				camPosThingX = bgStreet.x + bgStreet.width / 2;
				camPosThingY = bgStreet.y + bgStreet.height / 2;
			...
			// FlxG.camera.setScrollBoundsRect(sky.x, sky.y, sky.width, sky.height);
			...
			case 'tank': // Week 7 - Ugh, Guns, Stress
				...
				camPosThingX = ground.x + ground.width / 2;
				camPosThingY = ground.y + ground.height / 2 + 50;
				defaultCamZoom = .8;
			...
			case 'bruhbg':
				...
				foreground.scale.set(1.2, 1.2);
				foreground.updateHitbox();
				foreground.setPosition(-460, -65);
				foreground.antialiasing = ClientPrefs.globalAntialiasing;
				add(foreground);

				var dedfuck:FlxSprite = new FlxSprite();
				...
				dedfuck.scale.set(1.5, 1.5);
				dedfuck.updateHitbox();
				dedfuck.setPosition(678, 475);
				add(dedfuck);
				...
				gruntoes.scale.set(1.8, 1.8);
				gruntoes.updateHitbox();
				gruntoes.setPosition(-790, 530);
		...
		// Character Repositioning per Stage
		switch (curStage)
		{
			case 'limo':
				boyfriend.x += 240;
		...
		// targetHBY = FlxG.height * 0.86;
		targetHBY = FlxG.height * 0.91;
		healthBarBG.scale.set(0.75, 0.75);
		healthBarBG.updateHitbox();
		healthBarBG.screenCenter(X);
		healthBarBG.scrollFactor.set();
		healthBarBG.visible = !ClientPrefs.hideHud;
		healthBarBG.xAdd = (-30 * healthBarBG.scale.x);
		healthBarBG.yAdd = (-28 * healthBarBG.scale.x);

		if (ClientPrefs.downScroll)
			targetHBY = 0.08 * FlxG.height;
		healthBarBG.y = targetHBY;

		healthBar = new FlxBar(healthBarBG.x + (healthBarBG.xAdd * -1), healthBarBG.y + (healthBarBG.yAdd * -1), RIGHT_TO_LEFT,
			Std.int(healthBarBG.width - (Math.abs(healthBarBG.xAdd * 2))), Std.int(healthBarBG.height - (Math.abs(healthBarBG.yAdd * 2))), this, 'health', 0,
			2);
		...
		var freePsychVer:FlxText = new FlxText(20, FlxG.height * 0.95, "FreePsych Engine Beta v0.1.0", 12);
		...
		scoreTxt.y = ClientPrefs.downScroll ? 10 : FlxG.height - scoreTxt.height - 10;
		add(scoreTxt);
		scoreTxt.text = '';
		...
		DiscordClient.changePresence(detailsText, "No leaks Jon -Phil Swift" + " (" + storyDifficultyText + ")", iconP2.getCharacter());
		#end
	...
	function startSong():Void
	{
		...
		DiscordClient.changePresence(detailsText, "No leaks Jon -Phil Swift" + " (" + storyDifficultyText + ")", iconP2.getCharacter(), true, songLength);
		#end
		setOnLuas('songLength', songLength);
		callOnLuas('onSongStart', []);
		openfl.system.System.gc(); // clear music //
	}
	...
	override public function onFocusLost():Void
	{
		...
		if (health > 0 && !paused)
		{
			DiscordClient.changePresence(detailsPausedText, "No leaks Jon -Phil Swift" + " (" + storyDifficultyText + ")", iconP2.getCharacter());
		}
		#end
	...
	override public function update(elapsed:Float)
	{
		...
		if (debugSelected < 0)
			debugSelected = debugObjects.length - 1;

		if (debugSelected > debugObjects.length - 1)
			debugSelected = 0;

		var curObj = debugObjects[debugSelected];

		if (FlxG.keys.justPressed.T)
			curObj.scale.set(curObj.scale.x - 0.05, curObj.scale.y - 0.05);
		if (FlxG.keys.justPressed.Y)
			curObj.scale.set(curObj.scale.x + 0.05, curObj.scale.y + 0.05);

		if (FlxG.keys.pressed.G)
			FlxG.keys.pressed.SHIFT ? curObj.y -= 5 : curObj.y--;
		if (FlxG.keys.pressed.V)
			FlxG.keys.pressed.SHIFT ? curObj.x -= 5 : curObj.x--;
		...
		FlxG.watch.addQuick('Object position', '${curObj.x}, ${curObj.y}');
		FlxG.watch.addQuick('Object scale', '${curObj.scale.x}');
		FlxG.watch.addQuick('Camera Zoom', defaultCamZoom);
		#end
		callOnLuas(' onUpdate ', [elapsed]);

		if (health != 2)
			healthBar.y = FlxMath.lerp(healthBar.y, targetHBY - healthBar.height / 2, 0.025);
		else
			healthBar.y = FlxMath.lerp(healthBar.y, ClientPrefs.downScroll ? targetHBY - healthBarBG.height * 3 : targetHBY + healthBarBG.height * 3, 0.05);
		...
		switch (curStage)
		{
			...
			case 'philly':
				...
				phillyCityLightsEvent.members[curLight].alpha -= (Conductor.crochet / 1000) * FlxG.elapsed * 1.5;
			case 'limo':
				if (!ClientPrefs.lowQuality)
				{
		...
		if (!inCutscene)
		{
			...
			if (!startingSong && !endingSong && boyfriend.animation.curAnim.name.startsWith(' idle '))
			{
				...
				{ // Kind of a mercy thing for making the achievement easier to get as it' s apparently frustrating to some playerss boyfriendIdled = true;
				}
			}
			else
			{
		...
		{
			//Conductor.songPosition += FlxG.elapsed * 1000;
			freestyle.DeltaTime.calculateDelta(elapsed);
	...
	function doDeathCheck(?skipHealthCheck:Bool = false)
	{
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead)
		{
			...
			if (ret != FunkinLua.Function_Stop)
			{
				...
				DiscordClient.changePresence("Game Over - " + detailsText, "No leaks Jon -Phil Swift" + " (" + storyDifficultyText + ")", iconP2.getCharacter());
				#end
				isDead = true;
				return true;
			}
	...
	function tweenCamIn(zoom:Float)
	{
	...
	private function popUpScore(note:Note = null):Void // possibly optimize this.
	{
		//return;
	...
	private function onKeyPress(event:KeyboardEvent):Void // WHAT THE FUCK IS THIS...
	{
		if (paused)
			return;
		//var eventKey = event.keyCode; 
		//var action:Null<String> = Controls.getActionFromKey(event.keyCode);
		//var binds = ["left","down","up","right"];
		var dir:Int = Controls.getKeyID(event.keyCode);
		//trace('Pressed: ' + dir);
		...
		if (dir != -1) {
			...
			var possibleNotes:Array<Int> = [];	// Possible Notes
			var inputNotes:Array<Note> = [];	// Possible Notes

			notes.forEachAlive(function(daNote:Note)
			{
				if (!daNote.isSustainNote && daNote.canBeHit && daNote.mustPress && !daNote.tooLate && !daNote.wasGoodHit) 
				{
					if (daNote.noteData == dir)
						inputNotes.push(daNote);
						//possibleNotes.push(daNote.noteData);
					
				}
			});
			inputNotes.sort(sortByShit);

			if (inputNotes.length != 0) {
				for (i in 0...inputNotes.length) // Based off Freestyle 1.2 Input
				{
					var daNote = inputNotes[i];
			...
			//var inputNotes:Array<Note> = obtainNotes(dir, false);
			//inputNotes.sort((a, b) -> Std.int(a.strumTime - b.strumTime)); // Sort by earliest

			/*if (inputNotes.length != 0)
			{ // IF THERE IS A NOTE TO HIT
				for (i in 0...inputNotes.length) // Based off Freestyle 1.2 Input
				{
					var daNote = inputNotes[i];
					if(keys[daNote.noteData]) 
			...
			var spr:StrumNote = playerStrums.members[dir];
		...
		//keysArray[i];
		//var dir = keysArray.indexOf(eventKey);
		//trace(eventKey, keysArray[0]);
	...
	private function obtainNotes(data:Int = -1, holds:Bool = false):Array<Note>
	{
		var inputNotes:Array<Note> = [];
		for (daNote in notes)
		{
			if (daNote.canBeHit
				&& daNote.alive
				&& !daNote.wasGoodHit
				&& !daNote.tooLate
				&& (data == -1 || daNote.noteData == data)
				&& (!holds ? !daNote.isSustainNote : daNote.isSustainNote))
			{
				inputNotes.push(daNote);
			}
		}
	...
	private function getKeyFromEvent(key:FlxKey):Int // WHY!? M I S E R Y
	{
		if (key != NONE)
		{
			for (i in 0...keysArray.length)
